...
export const useDeactivateProductListing = () => {
  const { connection } = useConnection();
  const wallet = useWallet();

  return useMutation({
    mutationFn: async (productListingId: string) => {
      if (!wallet.publicKey) {
        throw new Error("Wallet not connected");
      }
      try {
        const response = await clickcrateApi.deactivateProductListing(
          productListingId,
        ...
        console.error("Error deactivating product listing:", error);
        if (axios.isAxiosError(error)) {
          if (error.response?.status === 401) {
            ...
            console.error(
              `Error deactivating product listing: ${error.message}`
            );
          }
        } else {
          console.error(
            "An unexpected error occurred while deactivating product listing"
          );
        }
        throw error;
      }
